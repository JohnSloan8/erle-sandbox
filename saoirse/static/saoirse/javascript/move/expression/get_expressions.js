function getAbsoluteCoordsOfMainExpressions() {
    ...
    Object.keys( expressionObject.rel.expressions ).forEach( function( exp ) {

        expressionObject.abs.expressions[ exp ] = getAbsoluteCoordsOfSingleExpression( 'expression', expressionObject.rel.expressions[ exp ] );

    });

}

function getAbsoluteCoordsOfVisemes() {

    Object.keys( expressionObject.rel.visemes ).forEach( function( exp ) {

        expressionObject.abs.visemes[ exp ] = getAbsoluteCoordsOfSingleExpression( 'visemes', expressionObject.rel.visemes[ exp ] );

    });

}

function getAbsoluteCoordsOfSingleExpression( expressionViseme, exp ) {

    let indExpRel = expressionObject.rel.expressions[ exp.name ];
    if ( expressionViseme !== 'expression' ) {

        indExpRel = expressionObject.rel.visemes[ exp.name ];
...
function getAbsoluteCoordsOfExpressionNow() {
    ...
    let absExpression = $.extend(true, {}, expressionObject.rel.expressions.blank);
